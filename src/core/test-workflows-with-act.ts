 ...
 * Usage: node --experimental-strip-types src/test-workflows-with-act.ts [workflow] [options]
 */
...
function showUsage(): void {
	...
	console.log("  --job <n>          Test specific job only");
	console.log(
		"  --platform <n>     Platform to use (ubuntu-latest, macos-latest, etc.)",
	);
...
function getAvailableWorkflows(): string[] {
	...
	try {
		...
		return files.map((f: string) => f.replace(`${workflowsDir}/`, ""));
	} catch (_error) {
		return [];
	}
}
...
function runActCommand(options: ActTestOptions): void {
	...
	let eventInputs: Record<string, unknown> = {};
	...
	if (matrix) {
		actArgs.push(
			"--matrix",
			typeof matrix === "string" ? matrix : JSON.stringify(matrix),
		);
	}
	...
	try {
		// Run act command
		let actProcess: ReturnType<typeof spawn>;
		if (isProtoCommand) {
		...
		actProcess.on("error", (error: Error) => {
